...
This guide explains how to use a Namecheap domain, Cloudflare, or ngrok so Least can keep a stable ChatGPT connector URL.

There are two different products hiding behind the phrase "one URL":

- Personal stable URL: one developer runs Least locally and keeps a stable URL such as `https://mcp.example.space/mcp`.
- Hosted relay for all users: every user gets a stable Least URL without managing Cloudflare. This requires a hosted service that routes each ChatGPT request to the correct user's local agent.
...
- A hosted relay at `mcp.example.space` where each local Least agent opens an outbound connection and authenticates. ChatGPT calls the relay URL, and the relay forwards each request to the right connected local agent.
...
Relay
  ...
  forwards MCP traffic to the correct user's local Least agent
```
...
```text
least.example.space       marketing/docs site
...
That installs the official Cloudflare binary into `~/.least/bin` on supported macOS, Windows, and Linux machines. You can also install `cloudflared` manually and keep it on PATH.

Authenticate:

```bash
~/.least/bin/cloudflared tunnel login
```

Create a named tunnel:

```bash
~/.least/bin/cloudflared tunnel create least-local
~/.least/bin/cloudflared tunnel route dns least-local local.example.space
```

Start Least with that stable hostname:

```bash
least stable \
  --root /absolute/path/to/your/repo \
  --hostname local.example.space \
  --tunnel-name least-local \
...
Server URL: https://local.example.space/mcp?least_token=replace-with-a-long-stable-token
...
mkdir -p ~/.least
chmod 700 ~/.least
$EDITOR ~/.least/cloudflare-tunnel-token
chmod 600 ~/.least/cloudflare-tunnel-token
```
...
Least MCP token       protects the /mcp endpoint that ChatGPT calls.
```

## Ngrok Free Dev Domain

Ngrok is the simpler personal stable URL for most users. A free ngrok account includes a dev domain, which can be saved once in Least and reused every time the local server restarts.
...
Name: Least
Connection: Server URL
Server URL: https://your-domain.ngrok-free.dev/mcp?least_token=replace-with-a-long-stable-token
Authentication: None / No Authentication
```
...
- Per-session Least token rotation.
...
```text
ChatGPT -> Cloudflare/relay -> user's local Least agent
```

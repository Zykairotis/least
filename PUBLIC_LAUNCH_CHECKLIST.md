...
Least is a local developer bridge. Treat public launch readiness as two separate gates:
...
Do not present Least as a fully reviewed public ChatGPT app until it has gone through the current app review flow.
...
```text
Use Least. Call server_config, then open_current_workspace with include_tree=false. Read README.md and summarize the project without editing files.
```

```text
Use Least. Create a small static site from PRODUCT.md by writing index.html, styles.css, and README.md. Verify with one targeted search.
```

```text
Use Least. Try to read .env. Explain why the request is blocked.
```

```text
Use Least. Run bash with pwd, then run bash with a blocked command. Report both outcomes.
```

## Security Gate

- Keep auth enabled for public tunnels.
- Keep `LEAST_BASH_MODE=safe` by default.
- Keep `LEAST_WRITE_MODE=workspace` only for agent mode.
...
For stable URLs, `least setup` must save enough profile state so future starts from the same workspace only need:
...
- Least is not an OS sandbox.
- Least does not guarantee a ChatGPT model can call MCP tools.
- Least does not change ChatGPT, Codex, or OpenAI quota behavior.
